...
local function MergeLayouts(mainLayout, newLayout)
  -- Merge buildings
  for size, list in pairs(newLayout.buildings) do
    if not mainLayout.buildings[size] then
      mainLayout.buildings[size] = {}
    end
    for _, pos in ipairs(list) do
      table.insert(mainLayout.buildings[size], {pos[1], pos[2]})
    end
  end

  -- Merge lines
  for _, line in ipairs(newLayout.lines) do
    table.insert(mainLayout.lines, {line[1], line[2], line[3], line[4]})
  end
end
...
local function RemoveLinesByIndices(layout, indices)
  -- Mark indices to remove in a set for O(1) lookup
  local toRemove = {}
...
local function EraseBuilding(x, z, size)
...
local function ToggleBuilding(x, z, size)
  local group = currentLayout.buildings[size]
  for i, pos in ipairs(group) do
    if pos[1] == x and pos[2] == z then
      EraseBuilding(x, z, size)
      return
    end
  end
  AddBuilding(x, z, size)
end

function ClearLayout()
  for _, group in pairs(currentLayout.buildings) do
    for i = #group, 1, -1 do
      table.remove(group, i)
    end
  end
...
function widget:Initialize()
  ...
  content:Add(MakeButton({
    ...
    tooltip = "Send a message about how to disable auto erase",
...
function widget:DrawScreen()
  ...
  if loadedData and loadedData.layout then
    ...
    gl.Text("Hold [ALT] while placing to merge with current layout.", (vsx - textWidth) / 2, vsy * 0.4-40, 18, "o")
  elseif drawingMode then --Drawing
      local vsx, vsy = gl.GetViewSizes()
      local textWidth = 19 * 18
      gl.Color(1, 1, 0.5, 1)
      gl.Text("Use [left mouse button] to place buildings. Hold [ALT] to place in area", (vsx - textWidth) / 2, vsy * 0.4, 18, "o")
      gl.Text("Use [right mouse button] to place lines. Hold [ALT] to remove crossed lines", (vsx - textWidth) / 2, vsy * 0.4-20, 18, "o")
      
  elseif allowTranslationByKeys then --Translation
      local vsx, vsy = gl.GetViewSizes()
      local textWidth = 19 * 18
      gl.Color(1, 1, 0.5, 1)
      gl.Text("Translate layout using [W][A][S][D] keys", (vsx - textWidth) / 2, vsy * 0.4, 18, "o")
  end
  ...
  if button == 1 then
    ...
    if loadedData and loadedData.layout then
      ...
      if not altMode then
        ...
        MergeLayouts(currentLayout, copy)
      end
...
function widget:MouseRelease(mx, my, button)
  ...
  elseif button == 3 and lineStart then
    ...
    if altMode then
      ...
      RemoveLinesByIndices(currentLayout, indices)
    else
  ...
  --translation in world space
    ...
    -- transfrom "d" to camera space
      ...
      if tx ~= 0 or tz ~= 0 then
        TranslateLayout(currentLayout, tx, tz)
      end
    end
  end  
end
...
function widget:DrawWorld()
  ...
  -- Render Lines
  gl.Color(0, 1, 0, 0.6) -- yellowish lines with some transparency
  gl.LineWidth(3)
  gl.BeginEnd(GL.LINES, function()
    for _, line in ipairs(currentLayout.lines) do
